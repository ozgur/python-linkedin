...
class XMLBuilder(object):
    ...
    def create_elements(self, **elements):
        return [self.create_element_with_text_node(tag_name, text_node) \
                for tag_name, text_node in elements.items()]
...
class LinkedIn(object):
    def __init__(self, api_key, api_secret, callback_url, gae = False):
        """
        LinkedIn Base class that simply implements LinkedIn OAuth Authorization
        and LinkedIn APIs such as Profile, Connection vs.

        @ LinkedIn OAuth Authorization
        ------------------------------
        In OAuth terminology, there are 2 tokens that we need in order to have
        permission to perform an API request.

        Those are request_token and access_token. Thus, this class basicly intends
        to wrap methods of OAuth spec. which are related of gettting request_token
        and access_token strings.

        @ Important Note:
        -----------------
        HMAC-SHA1 hashing algorithm will be used while encrypting a request body
        of an HTTP request. Other alternatives such as 'SHA-1' or 'PLAINTEXT' are
        ignored.

        @Reference for OAuth
        --------------------
        Please take a look at the link below if you have a basic knowledge of
        HTTP protocol
        - http://developer.linkedin.com/docs/DOC-1008

        Please create an application from the link below if you do not have an API
        key and secret key yet.
    ...
    def request_token(self):
        """
        Performs the corresponding API which returns the request token in a query
        string.
        """
        self.clear()
        method = 'GET'
        ...
        self._calc_signature(self._get_url(relative_url), query_dict,
                             self._request_token_secret, method)
        ...
        self._request_token_secret = self._get_value_from_raw_qs(
            'oauth_token_secret', response)
        return True

    def access_token(self, request_token=None, request_token_secret=None,
                     verifier=None):
        """
        Performs the corresponding API which returns the access token in a query
        string.

        Accroding to the link (http://developer.linkedin.com/docs/DOC-1008),
        POST Querydict must include the following:
        * oauth_consumer_key
        * oauth_nonce
        ...
        * oauth_version
        ...
        self._request_token_secret = request_token_secret and request_token_secret \
                                     or self._request_token_secret
        ...
        if self._request_token is None:
            raise OAuthError(
                'There is no Request Token. Please obtain "request_token" first.')

        if self._request_token_secret is None:
            raise OAuthError(
                'There is no Request Token Secret. Please obtain "request_token first".')

        if self._verifier is None:
            raise OAuthError(
                'There is no Verifier Key. Please obtain "request_token" method.')

        method = 'GET'
        relative_url = '/uas/oauth/accessToken'
        query_dict = self._query_dict({'oauth_token' : self._request_token,
        ...
        self._access_token_secret = self._get_value_from_raw_qs('oauth_token_secret',
                                                                response)
        return True

    def get_profile(self, member_id=None, url=None, fields=[]):
        """
        Gets the public profile for a specific user. It is determined by his/her
        member id or public url. If none of them is given, the information of the
        application's owner is returned.
        ...
        Examples:
        client.get_profile(merber_id = 123, url = None, fields=['first-name', 'last-name'])
        client.get_profile(merber_id = None, url = None, fields=['first-name', 'last-name'])
        client.get_profile(member_id = None, 'http://www.linkedin.com/in/ozgurv')
    ...
    def get_connections(self, member_id=None, public_url=None, fields=[]):
        """
        Fetches the connections of a user whose id is the given member_id or url
        is the given public_url. If none of the parameters given, the connections
        of the current user are fetched.

        @Returns: a list of Profile instances or an empty list if there is no
        connection.
    ...
    def get_search(self, parameters):
        """
        Use the Search API to find LinkedIn profiles using keywords, company, name,
        or other methods. This returns search results, which are an array of
        matching member profiles. Each matching profile is similar to a mini-profile
        popup view of LinkedIn member profiles.
        """
        self._check_tokens()
        try:
    ...
    def send_message(self, subject, message, ids=[], send_yourself=False):
        """
        Sends a Non-HTML message and subject to the members whose IDs are given as
        a parameter 'ids'. If the user gives more than 10 ids, the IDs after 10th
        ID are ignored.

        @Input: string x string x list x bool
        @Output: bool
        Returns True if successfully sends the message otherwise returns False.
        Important Note: You can send a message at most 10 connecti=ons at one time.
        ...
        body = builder.xml()
        try:
            response = self._do_normal_query('/v1/people/~/mailbox', body=body,
                                             method='POST')
            if response:
    ...
    def send_invitation(self, subject, message, first_name, last_name, email):
        """
        Sends an invitation to the given email address. This method is very similiar
        to the 'send_message' method.
        ...
        first_name_element = builder.create_element_with_text_node('first-name',
                                                                   first_name)
        last_name_element = builder.create_element_with_text_node('last-name',
                                                                  last_name)
        builder.append_list_of_elements_to_element(
            person_element, [first_name_element, last_name_element])
        recipient_element.appendChild(person_element)
        recipients_element.appendChild(recipient_element)
        ...
        connect_type_element = builder.create_element_with_text_node('connect-type',
                                                                     'friend')
    ...
    def set_status(self, status_message):
        """
        This API is deprecated and should be replaced with the share status of
        linkedin. Issues a status of the connected user. There is a 140 character
        limit on status message. If it is longer than 140 characters, it is shortened.
        ...
        * We can only set status for the user who grants us access.
        ...
        body = builder.xml()
        try:
            response = self._do_normal_query('/v1/people/~/current-status',
    ...
    def clear_status(self):
        """
        This API is deprecated and should be replaced with the share status of
        linkedin. Clears the status of the connected user.
    ...
    def share_update(self, comment=None, title=None, submitted_url=None,
        ...
        """
        Use the Share API to have a member share content with their network or with
        all of LinkedIn.
        ...
        * We can only share items for the user who grants us access.
        ...
        if len(comment) > 0:
            comment_element = builder.create_element_with_text_node('comment',
                                                                    comment)
        ...
        if (submitted_url is not None) or (title is not None):
            ...
            if submitted_url is not None:
                submitted_url_element = builder.create_element_with_text_node(
                    'submitted-url', submitted_url)
                content_element.appendChild(submitted_url_element)

                if submitted_image_url is not None:
                    siu_element = builder.create_element_with_text_node(
                        'submitted-image-url', submitted_image_url)
                    content_element.appendChild(siu_element)
            ...
            if description is not None:
                description_element = builder.create_element_with_text_node(
        ...
        body = builder.xml()
        try:
            response = self._do_normal_query('/v1/people/~/shares', body=body,
                                             method='POST')
            if response:
                self._error = self._parse_error(response)
                return False
    ...
    def get_authorize_url(self, request_token=None):
        ...
        return '%s%s?oauth_token=%s' % (self.BASE_URL, '/uas/oauth/authorize',
                                        self._request_token)
    ...
    def _generate_nonce(self, length=20):
        return ''.join([string.letters[random.randint(0, len(string.letters) - 1)] \
    ...
    def _urlencode(self, query_dict):
        keys_and_values = [(self._quote(self._utf8(k)), self._quote(self._utf8(v))) \
    ...
    def _signature_base_string(self, method, uri, query_dict):
        return "&".join([self._quote(method), self._quote(uri),
                         self._quote(self._urlencode(query_dict))])
    ...
    def _do_normal_query(self, relative_url, body=None, method='GET', params=None):
        ...
        query_dict['oauth_signature'] = self._calc_signature(
            self._get_url(relative_url), signature_dict, self._access_token_secret,
            method, update=False)
    ...
    def _check_tokens(self):
        if self._access_token is None:
            self._error = 'There is no Access Token. Please obtain "access_token".'
            raise OAuthError(self._error)
        if self._access_token_secret is None:
            self._error = 'There is no Access Token Secret.'
            raise OAuthError(self._error)
    ...
    def _calc_signature(self, url, query_dict, token_secret, method='GET',
                        update=True):
        query_string = self._quote(self._urlencode(query_dict))
        signature_base_string = '&'.join([self._quote(method), self._quote(url),
                                          query_string])
    ...
    def _https_connection(self, method, relative_url, query_dict, body=None):
        if self._gae:
            ...
            return self._https_connection_regular(method, relative_url, query_dict,
                                                  body)
